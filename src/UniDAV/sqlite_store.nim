...
proc migrate(db: var Connection) =
  db.execute("PRAGMA foreign_keys = ON")
  db.execute("PRAGMA busy_timeout = 5000")
  discard db.value("PRAGMA journal_mode = WAL")
  db.execute("CREATE TABLE IF NOT EXISTS schema_meta (version INTEGER NOT NULL)")
  if db.value("SELECT COUNT(*) FROM schema_meta") == "0":
    db.execute("INSERT INTO schema_meta(version) VALUES (1)")
  let schemaVersion = parseInt(db.value(
  ...
  db.execute("""CREATE TABLE IF NOT EXISTS sync_journal (
  ...
  if schemaVersion < 2:
    ...
    try:
      db.execute("ALTER TABLE sync_journal ADD COLUMN content_type TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE sync_journal ADD COLUMN raw_data TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE sync_journal ADD COLUMN base_etag TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE sync_journal ADD COLUMN next_attempt_at TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE sync_journal ADD COLUMN created_at TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE sync_journal ADD COLUMN updated_at TEXT NOT NULL DEFAULT ''")
  ...
  if schemaVersion < 3:
    db.execute("BEGIN IMMEDIATE")
    try:
      db.execute("ALTER TABLE sync_journal ADD COLUMN base_data TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE conflicts ADD COLUMN operation_id TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE conflicts ADD COLUMN remote_etag TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE conflicts ADD COLUMN remote_content_type TEXT NOT NULL DEFAULT ''")
      db.execute("ALTER TABLE conflicts ADD COLUMN remote_deleted INTEGER NOT NULL DEFAULT 0")
      db.execute("CREATE UNIQUE INDEX IF NOT EXISTS conflicts_operation_id ON conflicts(operation_id) WHERE operation_id<>''")
...
proc addAccount*(store: SqliteStore; name, baseUrl: string): int64 =
  ...
  store.db.execute(
    "INSERT INTO accounts(name, base_url, created_at) VALUES (?, ?, ?)",
...
proc addCollection*(store: SqliteStore; accountId: int64; href, kind,
  ...
  store.db.execute(
    "INSERT INTO collections(account_id, href, kind, display_name) VALUES (?, ?, ?, ?)",
...
proc listAccounts*(store: SqliteStore): seq[AccountRecord] =
  for row in store.db.rows("SELECT id, name, base_url, state, created_at FROM accounts ORDER BY id"):
...
proc getAccount*(store: SqliteStore; accountId: int64): Option[AccountRecord] =
  let row = store.db.row("SELECT name, base_url, state, created_at FROM accounts WHERE id=?",
    accountId)
  if row.len == 0: return none(AccountRecord)
...
proc setAccountState*(store: SqliteStore; accountId: int64; state: string) =
  ...
  store.db.execute("UPDATE accounts SET state=? WHERE id=?", state, accountId)

proc listCollections*(store: SqliteStore; accountId: int64): seq[
    CollectionRecord] =
  for row in store.db.rows("""SELECT id, href, kind, display_name, sync_token
...
proc activeJournalCountForAccount*(store: SqliteStore; accountId: int64): int =
  parseInt(store.db.value("""SELECT COUNT(*) FROM sync_journal j
...
proc putResource*(store: SqliteStore; resource: ResourceRecord) =
  store.db.execute("""INSERT INTO resources(collection_id, href, etag, content_type, raw_data, deleted, updated_at)
...
proc getResource*(store: SqliteStore; collectionId: int64;
  ...
  let row = store.db.row("SELECT etag, content_type, raw_data, deleted FROM resources WHERE collection_id=? AND href=?",
    collectionId, href)
  if row.len == 0: return none(ResourceRecord)
...
proc listResources*(store: SqliteStore; collectionId: int64;
  ...
  let query = if includeDeleted:
      "SELECT href, etag, content_type, raw_data, deleted FROM resources WHERE collection_id=? ORDER BY href"
    else:
      "SELECT href, etag, content_type, raw_data, deleted FROM resources WHERE collection_id=? AND deleted=0 ORDER BY href"
  for row in store.db.rows(query, collectionId):
...
proc checkpoint*(store: SqliteStore; collectionId: int64; syncToken: string) =
  store.db.execute("UPDATE collections SET sync_token=? WHERE id=?", syncToken, collectionId)

proc syncToken*(store: SqliteStore; collectionId: int64): string =
  store.db.value("SELECT sync_token FROM collections WHERE id=?", collectionId)

proc schemaVersion*(store: SqliteStore): int =
  parseInt(store.db.value("SELECT version FROM schema_meta LIMIT 1"))
...
proc applyRemoteBatch*(store: SqliteStore; collectionId: int64;
  ...
  try:
    for change in changes:
      let active = store.db.value("""SELECT COUNT(*) FROM sync_journal
...
proc journalEntry(row: seq[string]): JournalEntry =
...
proc getJournalEntry*(store: SqliteStore; operationId: string): Option[JournalEntry] =
  let row = store.db.row("""SELECT operation_id, collection_id, operation, href, state,
  ...
  if row.len == 0: return none(JournalEntry)
...
proc enqueueOperation(store: SqliteStore; entry: JournalEntry) =
  ...
  store.db.execute("""INSERT INTO sync_journal(operation_id, collection_id, operation, href,
...
proc coalesceLocalUpsert*(store: SqliteStore; operationId: string; collectionId: int64;
  ...
  try:
    ...
    store.putResource(ResourceRecord(collectionId: collectionId, href: href,
      ...
      deleted: false))
    store.db.execute("""UPDATE sync_journal SET content_type=?, raw_data=?, state='pending',
...
proc coalesceLocalDelete*(store: SqliteStore; operationId: string; collectionId: int64;
  ...
  try:
    ...
    if entry.operation == jokCreate:
      store.db.execute("""UPDATE sync_journal SET state='complete', last_error='',
        next_attempt_at='', updated_at=? WHERE operation_id=?""", $now().utc,
          entry.operationId)
    else:
      store.db.execute("""UPDATE sync_journal SET operation='delete', state='pending',
...
proc coalesceLocalRestore*(store: SqliteStore; operationId: string; collectionId: int64;
  ...
  try:
    ...
    if entry.baseData == rawData:
      ...
      store.db.execute("""UPDATE sync_journal SET operation='replace', state='pending',
        content_type=?, raw_data=?, attempt=0, last_error='', next_attempt_at='', updated_at=?
...
proc nextJournalEntryForAccount*(store: SqliteStore; accountId: int64;
  ...
  let effectiveTime = if currentTime.len > 0: currentTime else: $now().utc
  let row = store.db.row("""SELECT j.operation_id, j.collection_id, j.operation,
...
proc markJournalState(store: SqliteStore; operationId: string; state: JournalState;
  ...
  store.db.execute(
    """UPDATE sync_journal SET state=?, last_error=?,
  ...
  if store.db.affectedRows != 1:
    raise newException(ValueError, "journal operation does not exist")
...
proc recoverInterruptedOperations*(store: SqliteStore;
  ...
  store.db.execute("""UPDATE sync_journal SET state='retry', last_error=?, next_attempt_at='',
...
proc conflictRecord(row: seq[string]): ConflictRecord =
...
proc getConflictForOperation*(store: SqliteStore; operationId: string): Option[
    ConflictRecord] =
  let row = store.db.row("""SELECT id, operation_id, collection_id, href, local_data,
  ...
  if row.len == 0: return none(ConflictRecord)
...
proc recordJournalConflict*(store: SqliteStore; entry: JournalEntry;
  ...
  try:
    store.db.execute(
      """UPDATE conflicts SET local_data=?,
    ...
    let affected = store.db.affectedRows
    if affected == 0:
      store.db.execute("""INSERT INTO conflicts(operation_id, collection_id, href, local_data,
        remote_data, remote_content_type, base_data, remote_etag, remote_deleted, resolved)
...
proc resolveConflictKeepRemote*(store: SqliteStore; operationId: string) =
  ...
  try:
    ...
    store.db.execute("UPDATE conflicts SET resolved=1 WHERE operation_id=?", operationId)
    store.markJournalState(operationId, jsComplete)
    store.db.execute("COMMIT")
  except CatchableError:
    store.db.execute("ROLLBACK")
...
proc resolveConflictRetryLocal*(store: SqliteStore; operationId: string) =
  ...
  store.db.execute("BEGIN IMMEDIATE")
  try:
    store.db.execute("""UPDATE sync_journal SET operation='replace', base_etag=?,
      ...
      $now().utc, operationId)
    store.db.execute("UPDATE resources SET etag=?, updated_at=? WHERE collection_id=? AND href=?",
...
proc resolveConflictMerge*(store: SqliteStore;
  ...
  store.db.execute("BEGIN IMMEDIATE")
  try:
    store.db.execute("""UPDATE sync_journal SET operation='replace', content_type=? ,
    ...
    store.db.execute("""UPDATE resources SET etag=?, content_type=?, raw_data=? ,
    ...
    store.db.execute("UPDATE conflicts SET resolved=1 WHERE operation_id=?",
      operationId)
    store.db.execute("COMMIT")
  except CatchableError:
    store.db.execute("ROLLBACK")
